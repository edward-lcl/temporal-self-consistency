...
Finally, consistent with prior findings that internal confidence signals outperform expressed ones, the model's own answer log-probabilities predict its correctness at AUROC 0.84 where the trained hedge output is at chance. We add that the *untuned* model's signal is stronger than the fine-tuned one's (0.8814 vs 0.8406): the procedure intended to install calibration degraded the calibration signal already present. We report this as evidence that the representation, not the optimiser, sets the ceiling — while noting that the recovered signal is statistically overwhelming yet practically weak (average precision 3.3× baseline), and so does not by itself constitute a working method.
...
- **A demonstration that a known metric flaw fired in practice, and a control protocol that catches it.** A constant confidence output, ignoring the input entirely, beats a *perfect* volatility classifier sixfold here. That ECE admits such constants is documented in the metrics literature (Section 2.7); what we contribute is a case where it silently produced a reported result, the condition under which it fires, and both regimes — valid and inverted — measured within one evaluation (Section 5.2).
...
- **Evidence that fine-tuning degraded a signal the model already had.** Prior work establishes that internal signals beat expressed confidence (Section 2.6); we show the expressed channel here is a *trained* one sitting at exactly chance within a relation, and that fine-tuning lowered the model's pre-existing log-probability signal from 0.8814 to 0.8406 while adding that chance-level channel. The representation, not the optimiser, sets the ceiling (Section 5.3).
...
### 2.6 Verbalized confidence and the internal/expressed gap

A body of recent work establishes that a language model's *expressed* confidence and its *internal* confidence signals come apart, and that the expressed channel is the weaker one. Instruct-tuned models in the 3–9B range report near-maximum confidence largely independently of correctness, with ceiling rates above 90% [Distilling Self-Consistency into Verbal Confidence, arXiv:2604.24070]. On the same items where that verbal channel is near chance, linear probes on hidden states reach AUROC 0.6–0.8, and single-pass logit entropy discriminates correctness substantially better than the verbal report. Related work probes internal representations directly for calibration signal [What LLM Forecasters Know but Don't Say] and asks mechanistically how verbal confidence is computed [arXiv:2603.17839], with the open question being whether it reflects self-evaluation or merely surface correlates of fluency.

**This directly anticipates our Section 5.3, and we position that result accordingly.** That log-probabilities outperform an expressed confidence channel is an established finding, not our discovery. Our contribution there is narrower and specific: we show the effect holds when the expressed channel is a *trained, discrete hedge vocabulary* rather than a verbalized natural-language statement, that the trained channel sits at exactly chance within a relation type while log-probabilities reach 0.84 on the same examples, and that fine-tuning *degrades* the model's pre-existing internal signal (0.8814 → 0.8406) while adding a channel with none. The phenomenon is known; the finding that a method designed to fix calibration destroys the calibration signal already present is what we add.

### 2.7 Critiques of ECE, and the constant-predictor failure

The limitations of Expected Calibration Error are well documented. ECE is sensitive to binning scheme, with too few bins hiding discrepancies and too many producing unstable estimates; it can change discontinuously under small perturbations of predictor outputs; the population quantity is not estimable from finite data without strong regularity assumptions; and averaging within bins allows overconfident and underconfident items to cancel [A comprehensive review of classifier probability calibration metrics, arXiv:2504.18278; Calibration through the Lens of Interpretability, arXiv:2412.00943].

Critically for this paper, **the constant-predictor failure is already known**: it is trivially possible to obtain excellent ECE by predicting according to the label distribution, without any per-item discrimination whatsoever.

**We therefore do not claim to discover that ECE can be gamed by a constant.** Our contribution is to show that this known flaw *actually fired* in a deployed research setting — that it produced a reported ~0.43 improvement which our controls attribute to the distance between two constant policies — and to give the specific condition under which it fires (Section 5.2: the ratio of asserted confidence to realised accuracy) together with a control protocol that catches it. The gap we address is between a critique that exists in the metrics literature and evaluation practice in applied papers, where ECE is still routinely reported without constant-policy baselines.

### 2.8 Temporal benchmark decay and self-updating designs

The staleness problem we measure in Section 4.7 is a recognised failure mode. RealTimeQA and FreshQA both depend on authors manually refreshing answers as the world changes, which bounds how well they support real-time evaluation, and performance decay on FreshQA-style Wikipedia-derived sets has been documented [FreshLLMs, arXiv:2310.03214]. PAT-Questions was designed specifically as a **self-updating** present-anchored benchmark to avoid exactly this [arXiv:2402.11034] — and is, notably, already named in our own Section 3.4 as a data source, though not used for the test split whose labels we find to be 98.7% expired.

Contamination-vs-recency is a separate documented axis: temporal QA sets built before a model's cutoff (TimeQA, TempReason, MenatQA) are likely present in pretraining data, while those built after (FreshQA, RealtimeQA, TAQA) are designed to resist it [Test of Time, arXiv:2509.00072]. UnSeenTimeQA takes a further step, constructing time-sensitive questions that cannot be answered from memorisation at all [arXiv:2407.03525].

**Our contribution here is measurement, not diagnosis.** That temporal gold labels decay is known and the self-updating remedy already exists. What we add is a quantified audit of one deployed dataset against a live source at a pinned timestamp — 98.7% of gold answers no longer current, 29.2% of test questions present in training with a different answer — and the observation that a *newer* model is penalised more heavily by it, since it is more likely to know the current value the label has not caught up to.

### 2.9 Benchmark auditing as a genre

This paper sits alongside recent work treating benchmarks as objects requiring validation rather than as instruments to be trusted. Item-response-theoretic audits estimate item difficulty and feasibility from response patterns; work on agentic benchmarks separates genuine capability gaps from broken tasks, verifier bypasses and infrastructure failures by applying an ordered validity screen with explicit reference and empty-solution controls, and reports an unresolved category rather than forcing every item into a verdict. We adopt that protocol directly — our oracle run and constant-policy controls in Section 4.4 are its analogues — and our claims ledger carries the same `UNCERTIFIED` outcome for cases the evidence cannot decide.
